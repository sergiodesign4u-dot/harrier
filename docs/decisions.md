...
## 2026-08-26 &middot; The second exam screen is deleted, and its second list was longer than the first

Node 6.1 Client summary draft was built in four states by an agent with clean context, from `handoff/docs/one-shot.md` alone, as the second of the two exam runs stage 13 requires. **The screen is deleted under branch B and the twenty two findings are kept**, which is the trade the run exists for.

**Why B.** The node has no specification, no route, no permission, no flow and **no control leading to it from any built screen in the product**. Accepting it would have knowingly repeated the P0 that the critique of node 7.1 had found forty minutes earlier: a screen that nothing reaches. The corpus returns to 66 pages across 14 screens, `coverage-map.mjs` agrees in both directions, and the `rail-out` focus ring debt returns from 9 pages to 5.

**The second list was longer, and the stage contract says a second list of the same length means the handoff is not ready.** Run 1 gave six gaps against the package; run 2 gave nine could-not-understand and thirteen understood-it-this-way. Taken as a number that is a failure. Taken apart it is not: **of the nine, three are the package and five are the IA and voice layers being genuinely empty for a deferred cluster**, which is a different finding and a real one. The count is recorded rather than explained away, and so is the analysis.

**What proves the fix worked is not the count, it is that run 1's six did not recur and the agent cites the block that closed them.** It reasoned proportions before copying a frame and refused the split it was copying. It shipped zero primary actions with an argument. It walked every `href` by hand "as the WHAT YOU COPY block requires". Run 1 made all three of those mistakes and noticed none.

**One gap is bigger than the exam.** `voice/docs/voice.md` is 474 lines and every one assumes the reader is at the console. Cluster 6 writes text **for a client's security contact who never signs in**, and for that reader every rule about person, register and coined vocabulary either does not apply or applies in reverse. The agent invented the split and said that it had invented it. That is a hole in the voice layer, not in the handoff, and it belongs to whoever opens cluster 6.

**The isolation rule is amended by what the run disclosed.** Five of the agent's greps used the mask `design/*.html`, which mechanically includes the four exam screens of run 1. It disclosed this itself, unprompted, and accounted for it: three produced aggregate counts with no filenames, two returned no rows, and the eight mandated check commands read the whole directory by construction. **The run stands, and the rule now says what it always meant: a match voids a run when a forbidden file is READ, not when a glob could have touched it.** Stated here because a rule that would have voided this run on a technicality would have thrown away twenty two findings to protect a formality.

## 2026-08-26 &middot; The freeze protected a record that had stopped being true

`wireframes/` had been frozen since stage 05 and three stages of instruments stood on it: the census of control forms, the count of composition instances that decides what is a pattern, every pixel baseline, and `diverge.mjs`, whose closing line still read "wireframes/ is frozen, so every difference was made on the coloured side". The owner asked whether the folder should be brought back into step with the product. The measurement is what settled it.

**41 of 62 paired pages differed in visible strings, and the differences were not drift of register.** They were the fixture cures. `Aubrey Dental Group` carried a ruling on seven grey surfaces for the one client the canon gives none, which is the dash the empty-baseline case on 4.8 stands on. `Norsk Marine` read `Real, contained` with an `acted` chip above a latitude of `Investigate`. The counts that stood on those rows stood with them: `Six moved`, `7 of 34 shown`, `6 pointers`. Twenty-one expansions still promised a control that was not there. Six reject screens rendered a second action in the pane foot while the dialog was open, because `.btn--primary-narrow` repainted the button at 360 and never hid it at the desk.

**A frozen record is worth freezing while it is true.** Once it asserts what the project has ruled false it stops protecting a record and starts protecting an error, and anyone opening `wireframes/log.html` read a decision for a tenant that has none. The folder is reconciled and the state the freeze protected is the tag `wireframes-frozen-04`, which is what the census, the pattern counts and the pixel baselines are reproducible against. The paired string diff went from 41 pages to 0.
