...
**One word, `leave`, and it leads into the kit.** This replaces the stage 06 wording rather than standing beside it. A value approved on a coloured screen is never carried anywhere by hand: `leave` means put the value in a variable in `design/kit/kit.css`, with its origin comment, where it reaches every screen by itself; put markup in the showcase `design/kit/kit.html`, from which every copy takes it; sync the row in `DESIGN.md`; sign the attribute in `design/concept/docs/concept.md`, and if the edit contradicts the attribute, change the attribute and say so out loud; then check **in a browser** that it arrived on the second screen and on every repeat of the component by itself. **A fix made on one screen is a desync, not a fix.** A value moved by hand is a second copy and the drift starts there. Two element-selector leaks from `research/_page.css` were found this way at 06, so the check ends in a browser and not in a read.
