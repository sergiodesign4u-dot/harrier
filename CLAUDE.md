...
**One word, `leave`, and it does five things.** A value approved on a coloured screen is not carried anywhere by hand: `leave` means update the variable in `design/_theme.css` with its origin comment, sync the row in `DESIGN-artifacts.md`, rebuild the touched section of `design/concept/concept.html`, sign the attribute in `design/concept/docs/concept.md` (and if the edit contradicts the attribute, change the attribute and say so out loud), then check in a browser that it reached the second screen and every repeat of the component **by itself**. A value moved by hand is a second copy, and the drift starts there. Two element-selector leaks from `research/_page.css` were already found this way at 06, so the check is a browser, not a read.
