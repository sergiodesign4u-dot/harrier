...
|---|---|---|---|
| **Contrast** | `node design/kit/checks/contrast.mjs design/` | Computed ratio of every text node against its **composited** ground, in both themes, at 1440 and at a measured 360 | **0 failures.** 528 exemptions, each with its reason declared, 0 page errors, 0 horizontal overflow |
| **Focus** | `node design/kit/checks/focus.mjs design` | A real `Tab` walk through the real tab sequence, in both themes, recording whether a ring **renders** and what it measures against the ground **behind** the control | **1 failure.** 1236 tab stops over 66 pages, 17 control types, both themes, 0 with no ring at all. `rail-out` measures 1.93:1 in dark |
