...
**H3.** `CONTESTED AT STAGE 02` If every Clerk output carries a provenance strip naming model, sources queried and time spent, then analysts will allocate attention on evidence rather than on order of arrival, because effort spent and confidence are different questions and only the first predicts how much checking is warranted [`benchmark.md`, Lichess `Depth 75` and `CLOUD`]. **Contested by follow-up research:** the RIT study found analysts "rarely mentioned using the fine-grained feature contribution graphs or the prediction uncertainty fields in a real-time setting", which is evidence that such a strip may not be read at all under time pressure. The first test is whether analysts consult effort-spent information when it is one glance away rather than one click away. See section 8.
...
**H6.** `CORRECTED AT STAGE 02` If a shift handoff is composed **continuously through the shift and closed at the end**, then less information will be lost across a 24/7 rotation, because no product across the thirteen examined addresses the handoff at all and the nearest pattern comes from an email client [`competitors.md`; Superhuman Daily Briefs]. That the handoff is the riskiest hour of the rotation is itself unverified `[?]`. **The original wording said composed at the end of every shift.** Follow-up research put the failure mode exactly there, so the hypothesis was corrected rather than confirmed. See section 8 and `aarrr.md`.
...
| The lead's own working conditions | not stated at all | **69% of SOCs still rely on manual or mostly manual processes to report metrics**, and **62% of SOC professionals say their organisation is not doing enough to retain top talent** [SANS 2025, Key Findings]. Both are used in `personas.md` for P2 and had no chain in this file until now |
