...
**4. Tenant switching resets context.** `[?]` Every change of client resets what counts as normal: which assets matter, which hours are odd, which service accounts are noisy. We further assume analysts batch by tenant where they can, to avoid paying this cost repeatedly. Both halves are unverified. This is the behaviour behind the open question in `competitors.md` about whether a merged queue helps or harms.

**5. Writing for the future auditor.** `[?]` Anything that might be questioned later gets over-documented; anything that will not gets nothing. The analyst is not documenting for themselves and knows it. This is why a verdict interface that asks for free-text justification gets either a paragraph or the word "benign", with nothing in between.
