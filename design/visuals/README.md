...
**They are attached as CSS masks rather than markup.** `wireframes/` is frozen and all 32 coloured pages that existed then **held a structural diff of exactly zero when stage 06 measured it**, which an `<svg>` element on a screen would have ended. That zero was a reading taken at a time rather than a standing property, and it no longer holds; `design/kit/checks/diverge.mjs` is what measures the gap now, and no part of it is an icon. `background:currentColor` on the mask keeps the inheritance, so a glyph still takes the severity or the accent of whatever it sits in.
