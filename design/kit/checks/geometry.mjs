...
const EXPECTED = [
  ...
  { on: /^nar$/, keys: ['paddingLeft','textIndent'],
    why: 'THE TWO INSTANCES ARE TWO FILLINGS, not one component measured twice. A narration that opens with a timestamp takes a hanging indent so its wrapped lines land on the sentence rather than under the time; a narration without one is a plain paragraph and takes none. The instrument samples the first instance on each side, and the stand bench leads with a timed line while the product page leads with an untimed one. Stage 13' },
  { on: /^hint$/, keys: ['_h'],
    why: 'THE HINT SHRINKS BEFORE THE ACTIONS DO, and in the product it is shrinking. A dialog footer holding a hint, a spacer and two buttons does not always fit on one line: on the escalate screens it broke and left Cancel alone at the top right with the primary at the far left of a second line, two actions in opposite corners of one footer. The hint now takes zero basis and wraps its own small print instead, so the pair stays together. The stand bench is wider and has no reason to wrap. Stage 13' },
  { on: /^btn--primary$/, keys: ['_h','paddingTop','paddingBottom'],
    why: 'THE VERDICT FOOT IS DELIBERATELY TALLER THAN EVERY OTHER FOOT, and the instrument finds the product instance inside one. A foot holding four buttons is the strip where the analyst rules on a case, and it was sharing its padding with the strip that holds a single `Open the log`; the primary takes the full first line there and a taller block with it. The stand shows the plain primary in its bench at 32 and the four button arrangement in its own section, so both are visible rather than one of them being the truth. Six screens carry the four button set. Stage 13' },
