...
| **`.qfoot kbd` and `.key` do the same job in two forms** | **Closed by the screen with the most keys in the product.** `keyboard` is node 0.5 and it uses `.key` throughout, because every key on it is a key you are being taught. `qfoot kbd` is a key you could press right now in the list under your hand. The map confirms the two forms are two jobs rather than folding them |
