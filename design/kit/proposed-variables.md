...
| **Status** | **in the kit unchanged, at the old value.** Changing it changes what a person sees, so it waits |
...
| **Open** | which controls get one. The product reads perfectly today with none, and design principle 5 puts density first, so **adding icons has to earn its space rather than assume it** |
| **Status** | undecided |
