...
**Two values moved after this file was first written and both are recorded here rather than only in the kit.** `--color-scrim` was `rgba(22,24,26,.28)`, stage 04's light-theme ink at 28 per cent and the one value in the product the remap could never see, because it was a literal `rgba()` rather than a variable. It is now `color-mix(in srgb, var(--color-ground) 78%, transparent)`, the product's own ground. And an icon set entered the system at step 4, sixteen glyphs then and **nineteen since 2026-08-27**, applied by CSS mask so that no screen gains an element. The set grew twice: `next` at stage 12, and `sun` with `moon` together, because the control they are drawn for has two states and a glyph standing for one of them would be half a control. **The mask half holds and no screen gained one.** The clause that stood beside it here, that the structural diff stays at zero, does not: it was a measurement taken at stages 06 and 07 rather than a standing property, and the colour moved on after the freeze while the grey could not follow. `design/kit/checks/diverge.mjs` is where that gap is read.

No light theme, when this was written. **It has one now, and since 2026-08-27 the analyst can reach it.** It arrived as a property of the token level rather than as a feature: every role and every state token was written twice at founding, in `:root` and in `[data-theme="dark"]`, and the pair is not a mirror because contrast is computed against the opposite ground. The debt that stood here was shipping it, and what paid it is one control in the top bar, one file that runs before the first frame, and one storage key shared with the stand. **The default did not move:** with nothing stored the console is dark, and `prefers-color-scheme` is never read, because a machine that happens to be light is not an argument that reopens the ground. What the switch found is the more useful half: the product's contrast sweep had been seeding a key only the stand read, so **134 of its 268 renderings were the dark theme wearing a light label**, and the first real pass returned one failure across 67 pages. `docs/decisions.md` carries both.
...
- **The scale is in `rem` and two of its five sizes are fluid.** `--size-lg` runs 18px to 20px and `--size-xl` 22px to 26px, and the two ends of each clamp are this product's own two widths: 1280, the declared minimum, and 1920, the top of the declared primary band. **The two floors moved up by one pixel each on 2026-08-27**, from 17 and 21, on a user decision: both ceilings were kept, so the ramps narrow rather than shift, and what gains is the width the product is most often read at rather than the widest one. Both middle terms now share the addend `0.875rem`, which is arithmetic rather than tidying and is worth watching: an addend that stops matching is the sign that one end of one ramp moved without the other. The three small sizes do not grow, because density is the feature and on a wider monitor this analyst wants more rows rather than larger ones. **A `font-size` is never switched at a point**, and the middle term of a clamp always carries a `rem` addend: a pure `vw` term takes the page out of the reader's zoom and fails WCAG 1.4.4.
