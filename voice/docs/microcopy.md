...
| `The record beside this list is a desk reading.` | all four | Z4 banner, `only-narrow` | The same shape as the log's narrow banner and the same reason: `CLAUDE.md` gives the phone one scenario. Never &sect;6 is why it argues from what the phone is for rather than from a node number |
...
| `This ceiling stands on no record at this tenant.` | `tenant-no-record` | Z5 `gnote` | 3.5 section 7 calls this the state that most deserves attention, and it renders as a sentence rather than as a warning colour. Principle 4 |
| `The ceiling is read here and it is not set here.` | all four | Z5 `gnote` | 3.5 section 4: readable, not settable. Principle 4 again, and **a disabled control was the alternative and was refused**, because 7.2 does not exist to be disabled |
| `Every change to a ceiling is an entry.` | all four | Z5 `gnote` | 3.5 section 9 puts the grant history in 5.1. Dictionary, **entry**: one row of the log, never `record` |
...
| `Two rulings went against Clerk here, and the ceiling followed them.` | `tenant-moved-down` | Z5 `gnote` | Principle 4, and Principle 5: `Clerk`, no article. It says what the machine's record cost it |
