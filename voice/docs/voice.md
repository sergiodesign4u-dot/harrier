...
**A third act was found at step 8 and it is not this one either.** `shift-close-failed` offered `Hold it locally` after a failed **seal**, and nothing is held locally there: the brief is simply left unsealed, and the incoming analyst opens it with the written half missing. It becomes **`Leave it unsealed`**, and the cost is stated before the control, because that is what the *Dangerous, before the press* rule asks for. Codex flagged this as a D8 divergence, which is the correct reading of the ruling as it stood: **the ruling named one act and the product had three.**

**On the trailing status slot, which two instruments flagged and which keeps its lower case.** Nine `.go` values live on 2.1 and they hold four kinds of thing: a destination (`open the queue &rarr;`), an absence (`nowhere to review`, `nothing to open`), a freshness (`frozen at the seal`), and a progress (`done`, `counting`). Both critique instruments read the destination as a D6 residue that the `Open <the destination>` ruling should capitalise.

**It stays lower case, and the reason is what the slot is.** It is not a control label: **the whole line is the control**, and the slot is its tail, saying what this line leads to or amounts to. Capitalising one of nine would put a button in a column of fragments. The **vocabulary** still obeys the ruling, `open the queue` and not `go to the queue` or `start on the queue`, so one destination keeps one name across the product even though the case differs with the slot. Written down because an instrument will flag it again.
...
**On the state chip vocabulary, and it is fixed upward.** 0.8 §6 closes the case chip set at six. The log and the case history render seven more, and they are not a break in the set: they are **a second taxonomy on a different axis.** A case chip says what is true of the case now. An entry chip says what the entry is. Both are closed lists and 0.8 must declare both, which this stage does rather than leaving the contradiction for a later reader. The entry set is `accepted`, `rejected`, `amended`, `escalated`, `superseded`, plus the three machine acts `opened`, `filed`, `acted alone`. **`upheld` leaves this set with the ruling above.**

**The three machine values lost the word `Clerk` at step 8, and a browser is what found it.** They were drawn as `Clerk opened the case`, `Clerk filed a verdict` and `Clerk acted alone`, and they were the only chips in the column that **wrapped to two lines** while `accepted`, `rejected`, `amended` and `escalated` sat in one. The chip rule here says one word, lower case; these were three and four words and capitalised. **A taxonomy this stage declared at step 3 broke a rule this stage wrote at step 4**, and neither step could see it, because the collision is between a closed list and a column width. The cell to the chip's left already names the actor on every row, so the word `Clerk` was buying nothing.
...
| simply, just, easy | Every one of them tells the reader the thing she is struggling with is not hard. **The ban is on the minimiser, not on the spelling:** `just filed` and `just escalated` in a `<title>` are the time adverb and they stay. A grep for this row returns them, which is the same class as `btn--primary-narrow` matching `primary` at step 6. **Match the sense, not the substring** |
...
| **Block heading, `h3`** | **Names the question the block answers, in her words.** Interrogative form where the block answers a question she has (`What happened`, `Where the case is now`); the plainest noun phrase where the block is a **reading rather than an answer** (`Evidence, 9 signals`, `Normal at this tenant`, `Who is on`). The test is whether she could say the heading out loud as what she came to this block for, not whether it starts with `What` | `What happened` &middot; `What Clerk filed` &middot; `What Clerk may do here, on this tenant` &middot; `What emptied it` &middot; `What cannot be done here` |
...
| **Prefilled value** | Say where the value came from and whether to touch it. **Whether to touch it may be carried by a visible control beside the field** rather than by the hint, as `Use a different email` does on the door. A field she did not fill reads as a field the product filled wrongly, and a prefilled field has no placeholder left to carry its own condition | `Your email is the one you came back with. No password here.` |
...
| **Provenance line** | The absolute count, the window, and the named sources. Count first, never a bare percentage. **Two cases drop the names and both must say why they are dropping them**: at 360, where the line compresses to the count and the window; and where the sources are exactly what did not survive, as on an expired or lost snapshot, where naming them would claim a reading the product no longer holds | `6 sources queried over 24h: Entra ID, Exchange audit, EDR, proxy, threat intel, tenant baseline.` |
